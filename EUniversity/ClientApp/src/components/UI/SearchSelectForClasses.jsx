...
const SearchSelectForClasses = ({
  ...
  return (
    <>
      ...
      {inputValue ===
      (isUser
        ? `${chosenItem.firstName} ${chosenItem.lastName}`
        : `${chosenItem.name}`) ? (
        ''
      ) : (
        <div
          className={`flex-col gap-1 z-10 w-full ${
            inputValue && !givenValue ? 'flex' : 'hidden'
          } max-h-24 ${
            givenValue ? 'max-w-[257px]' : 'max-w-[200px]'
          }  overflow-y-auto scrollbar-hide absolute bg-background rounded-lg text-text shadow-lg p-2`}
        >
          {inputValue
            ? foundItems.length
