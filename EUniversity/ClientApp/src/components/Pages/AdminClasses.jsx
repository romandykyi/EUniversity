...
const AdminClasses = () => {
  ...
  const [searchLink, setSearchLink] = useState(
    `/api/classes?Page=${page}&PageSize=${pageSize}&name=${inputValue}&sortingMode=${sortingMethod}`,
  );
  ...
  useEffect(() => {
    const teacher = searchClass.teacher
      ? `&TeacherId=${searchClass.teacher}`
      : '';
    const group = searchClass.group ? `&GroupId=${searchClass.group}` : '';
    const classroom = searchClass.classroom
      ? `&ClassroomId=${searchClass.classroom}`
      : '';
    const classType = searchClass.classType
      ? `&ClassTypeId=${searchClass.classType}`
      : '';
    ...
    <PageForm
      ...
      tableBody={classes.map((item) => (
        ...
        >
          ...
          <td>{item.duration}</td>
          <td>
            {item.group.course.semester
              ? item.group.course.semester.name
              : 'no semester'}
          </td>
          {isAdmin ? (
            <>
