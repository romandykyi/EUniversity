...
const AdminClass = () => {
  ...
  const getTimeDifference = (targetDate) => {
    ...
    const hoursDif = Math.floor(
      (difference % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60),
    );
    const minutesDif = Math.floor(
            ...
            >
              <tbody>
                <tr>
                  ...
                  <td className='flex items-start text-3xl font-medium'>
                ...
                <tr>
                  ...
                  <td className='flex items-start text-3xl font-medium'>
                    {classItem.group.teacher.firstName}{' '}
                    {classItem.group.teacher.lastName}
                  </td>
                </tr>
                <tr>
                  <td className='text-3xl font-medium'>duration: </td>
                  <td className='flex items-start text-3xl font-medium'>
                    {classItem.duration.slice(0, -3)} min
                  </td>
                </tr>
                <tr>
                  <td className='text-3xl font-medium'>start at: </td>
                  <td className='flex items-start text-3xl font-medium'>
                    {convertTimeFormat(classItem.startDate)}
                  </td>
                </tr>
                <tr>
                  <td className='text-3xl font-medium'>before start: </td>
                  <td className='flex items-start text-3xl font-medium'>
                    {timeString ? timeString : 'loading...'}
                  </td>
                </tr>
              </tbody>
            </table>
