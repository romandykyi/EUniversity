...
const PageForm = ({
  ...
  return (
    <>
      <AddItemModal
        ...
        title={
          registerTitle === 'users'
            ...
            ? registerTitle.slice(0, -2)
            : registerTitle.slice(0, -1)
        }
      ...
      {additionalComponents}
      <PageOfItems
        title={`${inputValue ? 'Found' : 'All'} ${
          usersType ? usersType : registerTitle
        } (${totalItems})`}
        ...
        additionalItems={
          <>
            {additionalItems}
            <SortSelect setSortingMethod={setSortingMethod} />
            <Search setInputValue={setInputValue} search={fetchItems} />
            {isAdmin && (
              <Button
                addStyles={`${registerTitle === 'students' ? 'hidden' : ''}`}
                onClick={() => {
