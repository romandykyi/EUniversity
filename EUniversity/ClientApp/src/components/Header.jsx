    ...
    return (
      <ul className=' flex flex-col justify-end gap-3'>
        <li
          ...
          className='bg-theme px-3 py-1 rounded-lg w-full inline'
        >
          <Link className='text-white text-2xl font-medium w-full block' to='/'>
            Home
          </Link>
        </li>
        {authNavList.map((link) => (
          <li key={link.title} onClick={() => setCollapsed(false)}>
            <Link
              className='text-text text-2xl font-medium px-3 py-1 w-full block'
              to={link.to}
            >
              {link.title}
            </Link>
  ...
  return (
    <header className='py-3 shadow-md bg-background'>
      <div className='container max-w-[1100px] flex items-center gap-4 justify-between'>
        <Link
          className='text-2xl font-bold overflow-hidden whitespace-nowrap text-ellipsis'
          to='/'
        >
          EUniversity
        </Link>
        <nav
          className={`z-20 shadow-lg flex flex-col justify-center px-4 bg-background w-[300px] h-full fixed transition-all top-0 duration-300 ${
            collapsed ? 'right-0' : 'right-[-100%]'
          }`}
        >
          {isAuthenticated ? isAdmin ? <AdminNav /> : <AuthNav /> : ''}
        </nav>
        <div className='flex items-center gap-4'>
          <div
            className={`bg-text flex rounded-full items-center p-2 cursor-pointer w-[60px] h-9 ${
          ...
          {isAuthenticated ? (
            <div
              className='flex items-center justify-between w-10 h-6 flex-col cursor-pointer z-20 '
